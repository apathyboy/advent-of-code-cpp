...
std::vector<cuboid> parse_cube_ops(const std::vector<std::string>& lines)
{
    ...
    std::transform(lines.begin(), lines.end(), ops.begin(), [](const std::string& str) -> cuboid {
        cuboid op;
        ...
        if (nums.size() != 6) { throw std::runtime_error("bad input"); }
...
public:
    ...
    int loc_to_index(int loc) const { return loc_to_index_.at(loc); }

    int index_to_loc(int index) const { return index_to_loc_[index]; }
};
...
int64_t part1(const std::vector<cuboid>& full_input)
{
    std::vector<cuboid> cubes;
    std::copy_if(full_input.begin(), full_input.end(), std::back_inserter(cubes), [](const cuboid& c) {
        return (
            std::abs(c.x1) <= 50 && std::abs(c.x2) <= 50 && std::abs(c.y1) <= 50 && std::abs(c.y2) <= 50
            && std::abs(c.z1) <= 50 && std::abs(c.z2) <= 50);
    });
    return do_cube_ops(cubes);
}

int64_t part2(const std::vector<cuboid>& full_input)
{
    return do_cube_ops(full_input);
}
...
int main()
{
    fmt::print("Advent of Code 2021 - Day 22\n");

    std::ifstream ifs{"puzzle.in"};

    auto input = parse_cube_ops(rs::getlines(ifs) | rs::to<std::vector>);
...
{
    std::stringstream ss;
    ss << R"(on x=10..12,y=10..12,z=10..12
on x=11..13,y=11..13,z=11..13
off x=9..11,y=9..11,z=9..11
on x=10..10,y=10..10,z=10..10)";

    auto input = parse_cube_ops(rs::getlines(ss) | rs::to<std::vector>);

    REQUIRE(39 == part1(input));
}

TEST_CASE("Can solve part 2 example")
{
    std::stringstream ss;
    ss << R"(on x=-5..47,y=-31..22,z=-19..33
...
off x=-93533..-4276,y=-16170..68771,z=-104985..-24507)";
    ...
    REQUIRE(2758514936282235 == part2(input));
}
